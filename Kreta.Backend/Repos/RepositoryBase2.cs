...
{
    public abstract class RepositoryBase2<TEntity> : IRepositoryBase<TEntity>
        where TEntity : class, IDbEntity<TEntity>, new()
    {
        private readonly DbContext _dbContext;
        private readonly DbSet<TEntity>? _dbSet;

        public RepositoryBase2(KretaContext dbContext)
        {
        ...
        public async Task<ControllerResponse> UpdateAsync(TEntity entity)
        {
            ...
            catch (Exception e)
            {
                ...
                response.AppendNewError($"{nameof(RepositoryBase2<TEntity>)} osztály, {nameof(UpdateAsync)} metódusban hiba keletkezett");
        ...
        public async Task<ControllerResponse> DeleteAsync(Guid id)
        {
            ...
            else
            {
                ...
                {
                    response.AppendNewError(e.Message);
                    response.AppendNewError($"{nameof(RepositoryBase2<TEntity>)} osztály, {nameof(DeleteAsync)} metódusban hiba keletkezett");
                    if (entityToDelete is not null)
        ...
        public async Task<ControllerResponse> CreateAsync(TEntity entity)
        {
            ...
            else
            {
                ...
                {
                    response.AppendNewError(e.Message);
                    response.AppendNewError($"{nameof(RepositoryBase2<TEntity>)} osztály, {nameof(CreateAsync)} metódusban hiba keletkezett");
                    response.AppendNewError($"{entity} osztály hozzáadása az adatbázishoz nem sikerült!");
                }
            }
            return response;
